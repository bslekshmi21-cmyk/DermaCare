...
import 'package:userapp/gemini_service.dart';
...
class _UserHomePageState extends State<UserHomePage>
  ...
  Future<void> _fetchRecommendedProducts() async {
    try {
      final user = supabase.auth.currentUser;
      if (user != null) {
        final d = await supabase.from('tbl_user').select('type_id').eq('user_id', user.id).single();
      ...
      if (levelRes.isNotEmpty) {
        ...
        query = query.eq('type_name', userSkinType!);
      }
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      TextButton(
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => Productdetail())),
...
Widget _buildAdviceTile(String text) {
  ...
  Widget _buildProductChip(Map p) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const Productdetail())),
      child: Container(
        width: 150,
